...
/** CODCAJA nullable + CUOTAS (vales ya no van al corte de caja). */
let nominaValesSchemaOk = false;

async function ensureNominaValesEmpleadosSchema(pool) {
  if (nominaValesSchemaOk) return;
  const tableCheck = await pool.request().query(`
    SELECT 1 AS ok
    FROM sys.tables
    WHERE name = 'NOMINA_VALES_EMPLEADOS' AND schema_id = SCHEMA_ID('dbo')
  `);
  ...
  await pool.request().query(`
    ...
    BEGIN
      ALTER TABLE dbo.NOMINA_VALES_EMPLEADOS
        ADD CUOTAS INT NOT NULL CONSTRAINT DF_NOMINA_VALES_CUOTAS_APP DEFAULT (1);
    END
  `);
  nominaValesSchemaOk = true;
}
...
/** Cuota sugerida a descontar en nómina (no supera el saldo). */
function cuotaSugeridaVale(vale) {
  const cuotas = Math.max(1, parseInt(vale?.CUOTAS, 10) || 1);
  const monto = roundMoney(vale?.MONTO);
  const saldo = roundMoney(vale?.SALDO ?? monto);
  if (saldo <= 0) return 0;
  return Math.min(saldo, roundMoney(monto / cuotas));
}

/** Vales con saldo pendiente (no cancelados) de un empleado. */
async function listValesPendientesEmpleado(pool, empnit, codemp) {
  const result = await pool
    .request()
    .input('EMPNIT', sql.VarChar, empnit)
    .input('CODEMP', sql.Int, Number(codemp))
    .query(`
      SELECT v.ID, v.EMPNIT, v.CODEMP, v.FECHA, v.MES, v.ANIO, v.MONTO,
             ISNULL(v.ABONOS, 0) AS ABONOS,
             ISNULL(v.SALDO, v.MONTO - ISNULL(v.ABONOS, 0)) AS SALDO,
             ISNULL(v.CUOTAS, 1) AS CUOTAS,
             v.DESCRIPCION, v.USUARIO, v.FECHA_CREACION
      FROM dbo.NOMINA_VALES_EMPLEADOS v
      WHERE v.EMPNIT = @EMPNIT
        AND v.CODEMP = @CODEMP
        AND ISNULL(v.SALDO, v.MONTO - ISNULL(v.ABONOS, 0)) > 0
      ORDER BY v.FECHA ASC, v.ID ASC
    `);
  return (result.recordset || []).map((r) => ({
    ...r,
    CUOTA_SUGERIDA: cuotaSugeridaVale(r),
  }));
}

/** Mapa CODEMP → { totalCuota, totalSaldo, vales[] } para precargar deducciones. */
async function mapValesPendientesPorEmpleado(pool, empnit) {
  const result = await pool
    .request()
    .input('EMPNIT', sql.VarChar, empnit)
    .query(`
      SELECT v.ID, v.CODEMP, v.FECHA, v.MES, v.ANIO, v.MONTO,
             ISNULL(v.ABONOS, 0) AS ABONOS,
             ISNULL(v.SALDO, v.MONTO - ISNULL(v.ABONOS, 0)) AS SALDO,
             ISNULL(v.CUOTAS, 1) AS CUOTAS,
             v.DESCRIPCION
      ...
      WHERE v.EMPNIT = @EMPNIT
        AND ISNULL(v.SALDO, v.MONTO - ISNULL(v.ABONOS, 0)) > 0
      ORDER BY v.CODEMP, v.FECHA, v.ID
    `);
  const map = new Map();
  for (const raw of result.recordset || []) {
    const cuota = cuotaSugeridaVale(raw);
    const row = { ...raw, CUOTA_SUGERIDA: cuota };
    const key = Number(raw.CODEMP);
    if (!map.has(key)) map.set(key, { totalCuota: 0, totalSaldo: 0, vales: [] });
    const bucket = map.get(key);
    bucket.totalCuota = roundMoney(bucket.totalCuota + cuota);
    bucket.totalSaldo = roundMoney(bucket.totalSaldo + Number(raw.SALDO || 0));
    bucket.vales.push(row);
  }
  return map;
}
...
async function createVale(pool, empnit, data) {
  await ensureNominaValesEmpleadosSchema(pool);
...
async function crearPagoVale(pool, empnit, idVale, data) {
  ...
  const desdeNomina =
    data.desdeNomina === true || String(data.ORIGEN || data.DESDE_NOMINA || '').toUpperCase() === 'NOMINA';
  const codcajaRaw = data.CODCAJA ?? row.CODCAJA;
  const codcaja = codcajaRaw == null || codcajaRaw === '' ? null : parseInt(codcajaRaw, 10);
